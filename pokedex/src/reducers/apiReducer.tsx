...
export const { fetchStart, fetchQuery, fetchSuccess, fetchFail } =
  dataSlice.actions;
...
export const fetchMoreData =
  () => async (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    try {
      ...
      const additionalDataPromises = pokemonData.map((pokemon: PokemonData) =>
        axios.get(pokemon.url)
      );
      const additionalDataResponses = await Promise.all(additionalDataPromises);

      const updatedPokemonData = pokemonData.map(
